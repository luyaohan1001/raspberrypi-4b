...
static struct of_device_id my_device_ids[] = {
    {
...
static int dt_probe(struct platform_device *pdev) {
    ...
    if (!device_property_present(dev, "label")) {
        prink("dt_probe - Error. Device property 'label' not found");
    }

    if (!device_property_present(dev, "my_value")) {
        prink("dt_probe - Error. Device property 'my_value' not found");
    }
...
static int dt_remove(struct platform_device *pdev) {
    prink("dt_remove - Entered dt_remove.\n");
    return 0;
}
